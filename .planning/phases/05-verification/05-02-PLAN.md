...
3. Read the debugger's output text. Find the line starting with `Status:` and match the value after the colon to determine the next action:
   - If the value is "Fixed" -- show "Found the cause... Applying fix (attempt N of max)... Fixed!" Then spawn `director-verifier` via Task tool to re-check the specific area. If re-check passes, run `git add -A && git commit --amend --no-edit` to include the fix in the task commit. Move to next issue.
   - If the value is "Needs more work" -- increment retry counter. If under max retries, show "Trying a different approach (attempt N of max)..." and loop back to step 1.
